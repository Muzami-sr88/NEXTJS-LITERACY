...
export default function CriticalPerspectivesInsights() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${API}/critical-perspectives?${params}`);
    ...
    <>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <h2 className="text-2xl md:text-3xl font-bold text-[#07294e] mb-5">
          {searchQuery ? `${total} search results for "${searchQuery}"` : `${total}+ Critical Perspectives Insights`}
        </h2>
            ...
            <div key={bucket}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                {grouped[bucket].map(item => (
                  <Link href={`/critical-perspectives/${item.slug}`} key={item._id || item.slug} className="relative rounded-lg border-2 border-[#b5d56a] bg-white p-6 cursor-pointer transition-all duration-300 hover:shadow-2xl flex flex-col max-h-[240px] overflow-hidden group">
                    {item.image_url ? <Image src={mediaUrl(item.image_url)} alt={item.title} width={64} height={80} unoptimized className="absolute right-3 top-3 w-16 h-20 rounded-md object-cover border border-gray-200 shadow-sm" /> : <div className="absolute -right-4 bottom-4 opacity-10 pointer-events-none"><Image src="/Logo Icons/Literary Palace SVG Icon-01.svg" alt="Literacy"     width={220} height={220} className="w-44 h-44 md:w-52 md:h-52" /></div>}
                    ...
                    <div className="flex-1 flex flex-col pl-6 relative pb-12">
                      <div className="mb-2"><span className="inline-block bg-[#b5d56a] text-[#07294e] text-xs font-semibold rounded-full px-3 py-1">{item.tag || item.category || item.region || 'Critical Guide'}</span></div>
                    ...
                    <button type="button" aria-label="Bookmark critical perspective" onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleBookmark(item); }} className="absolute right-4 bottom-4 z-20 bg-white p-2 rounded-full shadow hover:bg-[#f9fce8]"><Bookmark className="w-5 h-5 text-[#07294e]" /></button>
                  </Link>
                ))}
              </div>
            </div>
