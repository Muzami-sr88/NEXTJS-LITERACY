...
export default function RecentlyAddedWorldLiterature() {
  ...
  useEffect(() => {
    ...
    async function loadRecent() {
      setLoading(true);
      try {
        const res = await fetch(`${API}/world-literature/recent?limit=${LIMIT}`);
  ...
  return (
    <>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {loading ? <div className="flex items-center justify-center py-12"><Loader2 className="w-7 h-7 animate-spin text-[#07294e]" /></div> : items.length === 0 ? <div className="py-12 text-center text-gray-500">No World Literature content found.</div> : <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {items.map(item => (
              <Link key={item._id || item.slug} href={`/world-literature/${item.slug}`} className="relative rounded-lg border-2 border-[#b5d56a] bg-white p-4 cursor-pointer transition-all duration-300 hover:shadow-2xl flex flex-col overflow-hidden min-h-[200px]">
                ...
                <div className="flex-1 flex flex-col pl-6 relative pb-10">
                  <div className="mb-2"><span className="inline-block bg-[#b5d56a] text-[#07294e] text-xs font-semibold rounded-full px-3 py-1">{item.tag || item.category || item.region || 'World Literature'}</span></div>
                ...
                <button type="button" aria-label="Bookmark world literature" onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleBookmark(item); }} className="absolute right-4 bottom-4 z-30 bg-white p-2 rounded-full shadow hover:bg-[#f9fce8]"><Bookmark size={20} className="text-[#07294e]" /></button>
              </Link>
            ))}
          </div>
        </>}
